...
The "target-ready" release: files are named the way emulators expect them, archives can be extracted for the systems that need it, and each target receives its covers and the metadata file it knows how to read. Validated first on Anbernic handhelds running dArkOS / ArkOS.
